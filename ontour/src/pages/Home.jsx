    ...
    for (var i = 0; i < artistIDs.length; i++) {
      try {
        var artistResponse = await fetch(
          `http://ec2-3-129-52-41.us-east-2.compute.amazonaws.com:8000/search_artist/${artistIDs[i]}`,
          { mode: 'cors' }
        )
        var artistData = await artistResponse.json()
        var artistId = artistData[0].artist_id
        var getReviews = await fetch(
          `http://ec2-3-129-52-41.us-east-2.compute.amazonaws.com:8000/reviews/${artistId}`,
          { mode: 'cors' }
        )
    ...
    <>
      ...
      <div id="homepage">
        <div id="homeheader">
          ...
          <div className="home-title">Own your next live experience.</div>
          <div className="search row">
            ...
            <button
              ...
              onClick={() => {
                alert('Feature coming soon! (see artists below)')
              }}
            >
          ...
          <div className="d-block d-sm-none">
            <div id="gallery" className="row">
              <div className="col-12 col-sm-9 align-self-center">
                <h4 className="fw-bold ">Recently Added Artists</h4>
              </div>
            </div>
            {artistIDs.map((item) => {
          ...
          <div className="d-none d-sm-block">
            ...
            {artistRows.map((item) => {
              return <div className="row mb-5">{generateRow(item)}</div>
            })}
          </div>
        </div>
      </div>
