...
const CommentBox = (props) => {
  ...
  const commentTable = "artist_comments";
  ...
  const postCountData = async (currCount) => {
    const { data, error } = await supabase
      .from(commentTable)
  ...
  return (
    ...
    >
      ...
      {props.imageData.event && (
        <div>
          <h4>
            {props.imageData.event} • {props.imageData.eventDate}
          </h4>
        </div>
      )}
      ...
      {showForm && (
        <form onSubmit={handleSubmit}>
          ...
          <Button variant="outlined" onClick={cancelComment}>
            Cancel
          </Button>
        </form>
      )}
