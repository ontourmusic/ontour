...
export default function Review(props) {
  ...
  const [isHelpfulActive, setIsHelpfulActive] = useState(
    isAuthenticated && props.likedUsers.includes(user.username)
  );
  const [isUnhelpfulActive, setIsUnhelpfulActive] = useState(
    isAuthenticated && props.dislikedUsers.includes(user.username)
  );
  ...
  const handleNotLogin = (event) => {
    alert("Please login to like or dislike a review.");
  };
  ...
  const sendNewReponse = async (newResponse) => {
    try {
      const { data, error } = await supabase
        .from(reviewTable)
        .update({ artist_response: newResponse })
        .eq("id", props.id); //add mutable artist id
  ...
  useEffect(() => {
    if (isAuthenticated && user && user.email) {
      ...
      if (
        ...
        user["https://tourscout.com/app_metadata"].username &&
  ...
  return (
    <div style={review_styles.item}>
      <div class="d-flex bd-highlight">
        ...
        <div class="p-1 bd-highlight">
          <h6 class="review-user">{props.user}</h6>
        </div>
        <br></br>
      </div>
      <div>
      ...
      {isRespondMode ? (
        ...
        <>
          <div>
            {props.response && (
              <div>
                <p className="response-title">Response from the Artist:</p>{" "}
                {/* Updated class name */}
                <p className="artist-response">{props.response}</p>{" "}
                {/* Updated class name */}
              </div>
            )}
          </div>
